...
void setup() {
  ...
  Mde.attach(motor_dianteiro_esquerdo);
  Mdd.attach(motor_dianteiro_direito);
  Mte.attach(motor_traseiro_esquerdo);
  Mtd.attach(motor_traseiro_direito);
...
void loop() {
  ...
  if (motor_ativo == 0)
  {
    ...
    // Map the pulse value from (544 -> 2400)us to (0 -> 180) degrees
    angle = map(targetPulseUs, 1000, 2000, 179, 0);
    // Send the motor command using servo lib
    Mde.write(angle);
  }
  if (motor_ativo == 1){
    targetPulseUs = pulse_Mdd;
    // Map the pulse value from (544 -> 2400)us to (0 -> 180) degrees
    angle = map(targetPulseUs, 544, 2400,179, 0);
    // Send the motor command using servo lib
    Mdd.write(angle);
  }
  if (motor_ativo == 2){
